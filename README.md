...
서울시 공공 버스 데이터를 조회하는 웹앱을 만들고, 그 과정에서 발생하는 사용자 행동 로그를 raw landing부터 분석용 데이터셋까지 연결한 Data Engineering 프로젝트입니다.
...
핵심은 HTTP 성공 ≠ 데이터 성공입니다. 응답 본문의 `headerCd`를 확인하고, 초당 제한(코드 23)처럼 쉬면 풀리는 오류만 지수 백오프로 재시도합니다.
...
실제 서울시 API에 연결해 수집·적재한 표본은 57건, 2세션입니다. 표본이 작으므로 사용자 행동의 일반 통계가 아니라 파이프라인이 어떤 정보를 드러내는지 보여주는 사례로 봐야 합니다.
...
- 총 수집 지연의 99% 이상이 브라우저 큐 대기 구간이었습니다(큐 대기 평균 2,557~2,780ms 대 전송 평균 3~4ms). 서버나 네트워크가 아니라 SDK의 전송 주기가 지배적이라는 뜻입니다. 지연을 한 숫자로 합쳤다면 보이지 않았을 사실입니다.
- 캐시 적중은 0~1ms, 외부 호출은 24~324ms로 관측됐습니다. §2의 정적/동적 분리가 실제로 무엇을 아꼈는지 보여주는 숫자입니다.
- 한 세션에서 목적이 3번 바뀌는 경로가 관찰됐습니다. `entry_intent`와 `current_intent`를 분리하지 않았다면 남지 않았을 기록입니다.
- `station_arrival`의 `data_age_sec`가 7회 모두 `null` 인 문제를 발견했습니다. 원인은 응답 정규화 단계에서 원본 수집 시각을 담지 않은 것이었습니다.

## 13. 실제 API를 붙여야만 나온 문제

목업만으로는 잡히지 않았던 것들입니다.

형제 API도 필드 의미가 다릅니다. 같은 기관, 같은 도메인이라도 오퍼레이션마다 규칙이 다릅니다. 정류소 도착정보의 `rerideNum1`은 혼잡도 코드가 아니라 재차인원(명)입니다. 혼잡도 코드로 읽으면 승객 1명이 '정보없음', 4명이 '보통'이 됩니다 — 척도 자체가 다릅니다.

단위가 명시된 값을 우선합니다. `traTime1`처럼 단위가 모호한 숫자 대신 `arrmsg1`의 `"3분12초후"`처럼 단위가 글자로 적힌 문자열을 파싱합니다. 명세의 항목 크기와 샘플 값이 서로 어긋나 해석이 갈렸기 때문입니다. `출발대기` · `운행종료` 같은 상태를 `0초`로 바꾸면 오지 않는 버스가 "곧 도착"으로 표시됩니다.

정류장은 이름으로 식별하면 안 됩니다. `능인선원앞`이 방향에 따라 `23365` / `23367`로 나뉩니다. 화면에는 똑같은 두 줄로 보이지만 반대편 정류장입니다. 그래서 식별자는 이름이 아니라 ARS 중심으로 관리하고, 조회 단계에서는 `station_id`와 `ars`를 함께 보존합니다.

실제로 클릭 이벤트의 ARS와 조회 응답의 내부 ID가 같은 `station_id` 이름을 써서 조인 결과가 0건인데 에러는 나지 않는 문제가 수집 로그에서 발견됐고, 스키마 `1.2.0`에서 정리했습니다.

원문의 오타도 그대로 따라야 합니다. 버스 위치 API의 혼잡도 필드는 `congetion`입니다. `congestion`으로 읽으면 혼잡도가 전부 비어서 나옵니다.

## 14. 합성 부하로 처리 성능 측정

사용자 행동 표본(57건)과 파이프라인 처리 성능을 혼동하지 않기 위해 별도로 측정했습니다. 수집 경로와 적재 경로를 실제 코드 그대로 통과시킵니다.
...
| queue/ingest 지연 분리 측정 | §7 시간 세 구간 |
...
전부 목업 모드에서 돌기 때문에 인증키 없이 CI에서 실행됩니다. CI가 초록불이라는 것은 clone한 사람도 바로 돌릴 수 있다는 뜻입니다.
...
저장소에는 원본인 `template.html`만 두고 생성물은 커밋하지 않습니다. 해설 문서는 `app/`·`sql/`의 실제 코드를 끼워 만들기 때문에, clone 직후 위 두 명령만으로 최신 코드 기준 문서가 나옵니다.
...
인증키는 `.env`에만 두고 브라우저로 전달하지 않습니다. 브라우저가 공공 API를 직접 못 부르는 이유이기도 합니다(키 노출 + CORS + http 혼합 콘텐츠). 그래서 서버가 중계합니다.
...
docs/           template.html — 해설 문서 원본 (build_docs 가 생성물을 만든다)
.github/        CI — 테스트 · SDK 점검 · 생성물 일치 검사
```
